...
- **OS**: Windows 10 / 11
- **配布版の実行環境**: `.NET 10 Runtime`（`app-framework-dependent` の場合）
...
※ `app-framework-dependent` の実行には `.NET 10 Runtime`（または `.NET 10 SDK`）が必要です。インストール状況が分からない、またはインストールしない場合は、`app-self-contained` フォルダをダウンロードしてください。

---

※srcからビルドを行う場合、以下のようにPower Shell スクリプトを実行してください。

- 非自己完結版(.NET 10 SDK をインストール済の場合)

```
.\publish.ps1 -CleanOutput
```

- 自己完結版(.NET 10 SDK をインストール済で、配布先には .NET を入れたくない場合)
